...
function juliaCosinus() {

  // grid
  for (var x = 0; x < canvas.width; x++) {
    for (var y = 0; y < canvas.height; y++) {
      // canvas to complex (inverted y-axis mapping)
      var zRe = cmaxX / (canvas.width / 2) * (x - canvas.width / 2);
      var zIm = cmaxX / (canvas.height / 2) * (canvas.height / 2 - y);
      ...
      while (Math.abs(zIm) < r_c && iter < maxIter) {
        // Calculate cos(z) = cos(zRe + i*zIm) = cos(zRe)cosh(zIm) - i*sin(zRe)sinh(zIm)
        var cosRe = Math.cos(zRe) * Math.cosh(zIm);
        var cosIm = - Math.sin(zRe) * Math.sinh(zIm);

        // f(z) = c * cos(z)
        var newZRe = cRe * cosRe - cIm * cosIm;
        var newZIm = cRe * cosIm + cIm * cosRe;
      ...
      const factor = iter / maxIter; // Normalize iterations into the range 0-1
