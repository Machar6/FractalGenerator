// Function to compute the Julia set with smooth coloring
function julia(x, y, c) {
    ...
    if (n < maxIterations) {
        ...
        const nu = n - Math.log(logZn / Math.log(2)) / Math.log(2);
  ...
  // Function to compute the Julia set without smooth coloring
  function juliaWithoutSmooth(x, y, c) {
    let zx = x, zy = y;
    let n = 0;
    while (zx * zx + zy * zy <= 4 && n < maxIterations) {
        const zx2 = zx * zx;
...
function getColor(iteration) {
  ...
  // Draw the Julia set with smooth or non-smooth coloring based on the switch state
  function drawJulia() {
    const zoom = 200;
    const shiftX = 0;
    const shiftY = 0;
    ...
    for (let x = 0; x < canvas.width; x++) {
        for (let y = 0; y < canvas.height; y++) {
            ...
            const zx = (x - canvas.height / 2 + shiftX) / zoom;
            const zy = (canvas.width / 2 - y + shiftY) / zoom;
  
            // Compute the iteration count
            const iteration = smooth
                ? julia(zx, zy, c)
                : juliaWithoutSmooth(zx, zy, c);
